...
public static function scandir2($dir)
{
    $array = scandir($dir);
    $array2 = array();
    foreach ($array as $a)
...
public static function getFullDossier($path, $relativePath)
{
    $docFull  = new DocFull();
    ...
    $docFull->tagsString = DocUtils::getTagsString($path);
    $docFull->files = DocUtils::getFiles($path, $relativePath);
    
    return $docFull;
}

public static function getFiles($path, $relativePath)
{
    $files = DocUtils::scandir2($path);
    $result = array();
    foreach($files as $f)
    {
        if ($f != "__tags.txt" && $f != "__description.txt")
        {
            ...
            $newDocFile->name = $f;
            $newDocFile->url = DocUtils::slugify(basename($relativePath).'/'.DocUtils::slugify($f));
            $newDocFile->isDir = is_dir($path.'/'.$f);
